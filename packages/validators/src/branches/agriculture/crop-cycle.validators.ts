...
export const createCropCycleSchema = z.object({
  ...
  plantingDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'Valid date is required' }),
  expectedHarvestDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'Valid date is required' }).optional(),
